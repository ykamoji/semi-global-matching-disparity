...
def showMatches(im1, im2, c1, c2, matches, title="", output_path=None):
    ...
    plot_matches(ax, im1, im2, c1[:, [1,0]].astype(int), c2[:,[1,0]].astype(int), disp_matches,
                 matches_color='dodgerblue', keypoints_color='k', only_matches=True)
    ax.set_title(title)
    plt.axis('off')
    if output_path:
        plt.savefig(output_path, bbox_inches='tight', edgecolor='auto')
    # plt.show()
